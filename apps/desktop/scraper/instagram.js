...
async function getUserStories(username) {
    ...
    try {
        ...
        // Download images before closing the browser session
            ...
            if (item.type === 'image') {
                try {
                    ...
                    if (response.ok()) {
                        ...
                        console.log(`[DOWNLOAD] CDN responded ${response.status()} for API URL — skipping local save.`);
                    }
                } catch (e) {
...
async function openProfile(username) {
    ...
    // Allow user to naturally quit out of the Chrome window
    page.on('close', async () => {
        try { await browser.close(); } catch (e) {}
    });
}

module.exports = { login, getPostMetadata, getHighlightMetadata, getUserStories, checkProfileAccess, downloadStoryImage, openProfile };
