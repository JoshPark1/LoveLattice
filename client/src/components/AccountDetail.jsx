          ...
          {(!account.trackedPosts || account.trackedPosts.length === 0) ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {account.trackedPosts.map((post) => {
                const thumbSrc = post.thumbnailUrl.startsWith('/thumbnails')
                  ? `${serverBase}${post.thumbnailUrl}`
                  : post.thumbnailUrl;
              ...
              <div className="flex-1 min-h-[150px] max-h-64 overflow-y-auto pr-2 space-y-2">
                {accountLogs.length === 0 ? (
                  ...
                  accountLogs.map((log) => {
                    const snapshotUrl =
                      log.storyThumbnail &&
                      log.storyThumbnail.startsWith('/thumbnails')
                        ? `${serverBase}${log.storyThumbnail}`
                        : log.storyThumbnail || log.storyUrl;
